# Created by xionghuichen at 2022/9/2
...
def is_terminal(obs, act, next_obs, task):
    if 'halfcheetahvel' in task:
        ...
        # sas = np.concatenate([obs,act,next_obs], axis=-1)
        # not_done = np.logical_and(np.all(sas <= max_val, axis=-1), np.all(sas >= min_val, axis=-1))
        # done = ~not_done
        # done = done[:, None]
        # return done
