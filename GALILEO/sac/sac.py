...
from GALILEO.sac.policies import SACPolicy
# from stable_baselines import logger
from RLA import logger
from RLA import exp_manager


def total_episode_reward_logger(rew_acc, rewards, masks, steps, name='episode_reward'):
    """
...
class SAC(OffPolicyRLModel):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    mean_q_target = tf.reduce_mean(q_backup)
                    ...
                    with tf.control_dependencies([policy_train_op]):
                        ...
                        self.infos_names = ['policy_loss', 'qf1_loss', 'qf2_loss', 'value_loss', 'entropy', 'q_target']
                        # All ops to call during one training step
                        self.step_ops = [policy_loss, qf1_loss, qf2_loss,
                                         value_loss, qf1, qf2, value_fn, logp_pi,
                                         self.entropy, mean_q_target,
    ...
    def learn(self, total_timesteps, callback=None,
        ...
        for step in range(total_timesteps):
            ...
            total_episode_reward_logger(self.episode_reward, ep_reward, ep_done, self.num_timesteps)
            ...
            end_episode = ep_step >= episode_len
            if done or end_episode:

                logger.record_tabular("perf/train_returns", episode_rewards[-1])
                if self.action_noise is not None:
                    self.action_noise.reset()
            ...
            if self.verbose >= 1 and (done or end_episode) and log_interval is not None and num_episodes % log_interval == 0:
